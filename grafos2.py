...
class Graph:
    def __init__(self):
        # Grafo con usuarios predefinidos y amistades
        self.adj_list = {
    ...
    def add_friendship(self, u1, u2):
        if u1 in self.adj_list and u2 in self.adj_list and u2 not in self.adj_list[u1]:
...
class SocialGraphApp:
    ...
    def setup_ui(self):
        ...
        tk.Label(panel, text="Seleccionar usuario:").pack(pady=10)
        self.selected_user = ttk.Combobox(panel, state="readonly", values=list(self.graph.adj_list.keys()))
        self.selected_user.pack(pady=5)

        tk.Button(panel, text="Mostrar Conexiones", command=self.show_info).pack(pady=10)
        ...
        self.output = tk.Text(panel, height=15, width=35)
    ...
    def show_info(self):
        ...
        self.output.insert(tk.END, f"Amigos de {user}:\n{', '.join(amigos)}\n\n")
        self.output.insert(tk.END, f"Sugerencias (BFS nivel 2):\n{', '.join(sugerencias)}\n")
    ...
    def draw_graph(self):
        ...
        for user, (x, y) in self.user_positions.items():
            self.canvas.create_oval(x-20, y-20, x+20, y+20, fill="#0d6efd")
            self.canvas.create_text(x, y, text=user, fill="white")
...
if __name__ == "__main__":
    root = tk.Tk()  # Usamos Tkinter directamente
